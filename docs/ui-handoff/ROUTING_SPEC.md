...
- The window hosts a **tab bar**. `Home` is always tab 0. Each open site workspace is a tab.
- Within a **site tab**, the left **SiteSidebar** switches sub-routes.
...
| Route | Screen | Notes |
|---|---|---|
| `/onboarding` | SCR-01 First-run wizard | Full-window; shown when no project exists or via "Add site". Steps: `/onboarding/project`, `/sign-in`, `/site`, `/test`, `/apps`. |
| `/` | SCR-02 Project Home | The Home tab. Projects, Auth profiles, Site workspaces. |
...
## Site-tab routes  (prefix `/site/:siteId`)
...
- `/site/:siteId/*` requires a project open and the site to exist.
...
The top menu bar (`Project · Account · Site · Scan · Snapshot · Window · Help`) mirrors in-context actions and OS conventions. It is never the only path to any action.
